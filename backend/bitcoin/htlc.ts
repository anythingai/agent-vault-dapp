...
export class BitcoinHTLC {
  ...
  createRedemptionTransaction(
    ...
    // Create signature hash for redemption path (IF branch)
    ...
    const signature = userKeyPair.sign(sighash);
    ...
    psbt.finalizeInput(0, () => ({ witnessScript: witnessStack }));
    const tx = psbt.extractTransaction();
  ...
  createRefundTransaction(
    ...
    // Create signature hash for refund path (ELSE branch)
    const sighash = psbt.__CACHE.__TX.hashForWitnessV0(
      0,
      htlcOutput.redeemScript,
      htlcUTXO.value,
    ...
    const signature = resolverKeyPair.sign(sighash);
    const signatureWithHashType = Buffer.concat([
      signature,
      Buffer.from([bitcoin.Transaction.SIGHASH_ALL])
    ]);
